...
\begin{lecture}
Un modèle qui annonce $98\,\%$ de réussite n'a rien annoncé tant qu'on ignore
sur quelles lignes ce nombre a été obtenu. Deux questions restent alors
ouvertes : que mesurer, puisqu'un seul pourcentage cache la maison qui résiste,
et à quoi comparer le résultat pour savoir s'il vaut mieux qu'une règle triviale.
\end{lecture}
...
Pour un test de taille $m_{\mathrm{te}}$, l'\rsym{exactitude}{exactitude} est
\[
...
La \rsym{confusion}{matrice de confusion} $M$, définie par $M_{ab}=\#\{i:c_i=a,\widehat c_i=b\}$,
localise les maisons confondues. Chaque classe reçoit ensuite son \rsym{rappelprec}{rappel}, sa
\rsym{rappelprec}{précision} et son \rsym{f1}{score $F_1$}. La \rsym{macro}{moyenne macro} donne le même poids à chaque
...
majoritaire, qui prédit toujours la maison la plus fréquente. Le second, le \rsym{permutation}{contrôle par permutation}, mélange
...
Un \rsym{cv}{pli} désigne un bloc d'observations. Dans une \rsym{cv}{validation croisée}
