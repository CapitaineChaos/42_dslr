...
La \rsym{descente}{descente de gradient} simultanée produit la suite
\[
  \thv^{[t+1]}=\thv^{[t]}-\alpha\nabla J(\thv^{[t]}),
  \qquad \alpha>0.
\]
Cette itération est la descente par lot appliquée à la
log-vraisemblance négative \cite[chap.~8]{boyd2004}.

Le gradient donne la direction locale de hausse du coût, son opposé la direction
locale de baisse. Le développement au premier ordre de
l'annexe~\ref{app:descent-direction} l'établit. La propriété vaut au voisinage du
point courant, et un pas trop grand sort du domaine où cette approximation décrit
la fonction.

La mise à jour est \emph{simultanée} : les $n+1$ composantes du nouveau vecteur
se calculent toutes à partir du même ancien vecteur. Un taux élevé fait remonter
le coût ou diverger ; un taux très petit allonge la convergence.
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{Deux allures du coût au fil des itérations : décroissance stable en
  trait plein, oscillations d'un pas trop agressif en pointillé. Courbes
  construites pour la figure ; le tracé du coût réel rejoint l'archive de
  l'expérience.}
\end{figure}
...
\[
  ...
  \quad\text{ou}\quad \|\nabla J(\thv^{[t]})\|_2<\eps.
\]
Le plafond d'itérations borne le temps de calcul ; le test sur $J$ ou sur le
gradient atteste la convergence.
...
Trois diagnostics coexistent, chacun sur sa propre question. Une baisse du coût
dit que l'algorithme avance. Une petite norme du gradient dit qu'il approche un
point stationnaire. Une bonne performance de validation dit que ce point produit
un modèle utile hors échantillon. Chacun se vérifie séparément.

Un \rsym{stationnaire}{point stationnaire} est un point où le gradient s'annule. Pour
$J(\theta)=\theta^2$, le point $0$ est stationnaire et donne le minimum. Pour
$J(\theta)=\theta^3$, le point $0$ est stationnaire alors que la fonction décroît
à sa gauche et croît à sa droite : c'est un point d'inflexion. Dans le problème
logistique, la convexité établie au chapitre précédent fait de tout point
stationnaire fini un minimum global.

\section{Influence du taux d'apprentissage dans le cas scalaire}

\begin{lecture}
Une fonction d'une seule variable suffit à montrer ce que règle $\alpha$ : la
longueur du pas. Trop court, il exige un grand nombre de tours ; trop long, il
fait sauter le paramètre par-dessus le minimum, d'un côté puis de l'autre, sans
jamais s'en rapprocher.
\end{lecture}
...
itérations successives et marquent l'ordre des calculs.}
...
\begin{lecture}
Deux formules mathématiquement égales se comportent différemment sur un
ordinateur, dont les nombres ont une taille et une précision finies. Les formes
stables maintiennent les exponentielles dans le domaine représentable et
travaillent sur le score plutôt que sur une probabilité arrondie à $0$ ou $1$.
\end{lecture}

Pour un score très négatif, $e^{-z}$ dépasse la capacité du format flottant. Une
sigmoïde stable utilise donc deux branches :
\[
  \sigma(z)=
  \begin{cases}
    1/(1+e^{-z}),&z\geq0,\\
...
\]
plutôt que de calculer $\log(\sigma(z))$ après arrondi de la probabilité à $0$
ou $1$. Cette identité résulte du remplacement de $p$ par $\sigma(z)$ dans la
log-perte et constitue la forme usuelle de la perte logistique
\cite[sec.~7.1]{murphy2022}.

La fonction \rsym{softplus}{softplus} demande elle-même une évaluation protégée, car $e^z$ déborde
pour un score très positif. La forme employée est
\[
  \operatorname{softplus}(z)=\log(1+e^z)=\max(z,0)+\log\bigl(1+e^{-|z|}\bigr),
\]
dont l'exponentielle porte toujours sur un nombre négatif ou nul.

\begin{resultat}[Contrôle du gradient]
Avant tout entraînement, comparer le gradient analytique à une \rsym{diffinie}{différence finie centrée} sur un cas test de faible dimension :
\[
  \frac{\partial J}{\partial\theta_j}\approx
  \frac{J(\thv+h e_j)-J(\thv-h e_j)}{2h}.
\]
Ce test détecte un signe inversé, un facteur $1/m$ oublié ou une transposition
mal placée. Sa portée s'arrête à la dérivation et au code du gradient.
\end{resultat}
