...
\begin{lecture}
Les notes du fichier ne sont pas utilisables telles quelles : des cases sont
vides, et deux colonnes voisines se mesurent parfois sur des échelles sans
rapport l'une avec l'autre. Trois opérations les rendent calculables --- boucher
les trous, ramener les colonnes à une échelle commune, ajouter une colonne
constante --- sous une règle unique : chacune s'apprend sur les seules lignes
d'apprentissage, puis s'applique inchangée à toutes les autres.
\end{lecture}
...
consiste à définir la valeur utilisée lorsqu'une mesure manque. \rsym{standardisation}{Standardiser}
...
vide demande donc une règle explicite, par exemple l'imputation par la \rsym{mediane}{médiane}, et la
...
\]
Le diviseur de l'\rsym{ecarttype}{écart-type}, $m_{\mathrm{tr}}$ ou
$m_{\mathrm{tr}}-1$, change le résultat de moins
...
Le \rsym{conditionnement}{conditionnement} mesure la sensibilité du calcul aux directions de
...
\begin{vigilance}[\rsymt{fuite}{Fuite de données}]
...
\]
Pour construire une partition \rsym{stratifie}{stratifiée}, on regroupe d'abord les indices par
...
\begin{lecture}
Un tirage au sort global peut vider une classe rare de l'un des deux
sous-ensembles. Stratifier consiste à tirer séparément à l'intérieur de chaque
classe : les proportions d'origine se retrouvent alors des deux côtés.
\end{lecture}
...
Un \rsym{hyperparam}{hyperparamètre} est une quantité que le protocole fixe avant
...
proportion d'apprentissage et une \rsym{graine}{graine}. La graine est un entier qui
