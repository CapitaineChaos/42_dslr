...
\begin{lecture}
Une matrice de design est le tableau numérique effectivement reçu par le modèle.
Chaque ligne décrit un élève ; chaque colonne décrit une caractéristique. Une
colonne supplémentaire remplie de $1$ permet d'ajouter le même terme constant au
score de tous les élèves.
\end{lecture}
...
la frontière.
Sans première coordonnée égale à $1$, le score serait
$\theta_1x_1+\theta_2x_2$ et la frontière passerait nécessairement par l'origine.
Avec $\theta_0$, elle devient
$\theta_0+\theta_1x_1+\theta_2x_2=0$ et peut se déplacer parallèlement à
elle-même. Le biais est un paramètre du modèle qui règle cette translation de la
frontière.
...
Une régression logistique ne définit aucune opération sur une valeur manquante.
Il faut donc choisir explicitement une règle, par exemple l'imputation médiane,
et conserver pour chaque variable la valeur d'imputation utilisée. Une ligne ne
doit être supprimée que si ce choix est justifié et documenté.

Exemple : les valeurs d'apprentissage d'un cours sont
$\{4,7,9,12,100\}$. La médiane vaut $9$ ; une note manquante est donc remplacée
par $9$. La moyenne vaudrait $26{,}4$ et serait fortement déplacée par la valeur
$100$. Ce calcul explique l'intérêt possible de la médiane, mais ne démontre pas
qu'elle est toujours optimale : l'imputation réduit artificiellement la
dispersion et peut masquer un mécanisme de données manquantes informatif.
...
et à toute observation future. Il contracte légèrement $s_j$, puisque les valeurs
imputées sont toutes égales à la médiane. Estimer $\mu_j$ et $s_j$ sur les seules
valeurs observées est l'autre convention défensible. Ce qui compte n'est pas le
choix retenu, mais son enregistrement et son application identique à chaque étape
de la chaîne.
...
\end{tabular}
\end{tabular}
\caption{Imputation médiane sur une variable. La valeur ajoutée est mise en
évidence : elle n'a pas été observée et ne doit pas être confondue avec une
mesure réelle. La médiane est calculée sur l'apprentissage seulement.}
...
\]
Le choix du diviseur ($m_{\mathrm{tr}}$ ou $m_{\mathrm{tr}}-1$) a peu d'effet
ici, mais doit rester identique entre entraînement et prédiction. Si $s_j=0$, la
variable est constante sur l'apprentissage et doit être retirée.

La standardisation ne modifie pas la famille des frontières linéaires : elle
reparamètre le modèle. Elle améliore en revanche le conditionnement numérique de
l'optimisation et rend une éventuelle pénalisation comparable entre variables
\cite[sec.~4.6.3]{hastie2009}.

Le \emph{conditionnement} décrit ici la sensibilité du calcul aux différentes
directions de l'espace des paramètres. Si une variable varie autour de $10^3$ et
une autre autour de $10^{-2}$, un même déplacement numérique de leurs
coefficients ne modifie pas le score dans des proportions comparables. Les
courbes de niveau du coût deviennent très allongées : la descente zigzague ou
nécessite un petit pas. La figure~\ref{fig:contours} en donne la représentation
géométrique ; la standardisation rapproche les échelles sans modifier
l'information portée par l'ordre des observations.
...
ordre et leurs positions relatives, mais fixe une origine et une échelle
communes. Une valeur standardisée de $1{,}5$ signifie « un écart-type et demi
au-dessus de la moyenne d'apprentissage » ; elle ne signifie ni $150\,\%$ ni une
probabilité de $1{,}5$.
\end{resultat}
...
l'apprentissage. L'évaluation devient alors optimiste. La partition doit
précéder toute transformation apprise des données.
\end{vigilance}
...
\begin{enumerate}
  ...
  \item conserver médiane, $\mu_j$ et $s_j$, puis les réutiliser sans les recalculer ;
...
obtenus sont enfin réunis dans les deux listes. Les données numériques restent
inchangées : seuls leurs indices sont répartis.

Les objets à conserver sont donc la graine et les deux listes d'indices. Les
moyennes, écarts-types et valeurs d'imputation sont ensuite calculés uniquement
sur les lignes désignées par $\mathcal I_{\mathrm{tr}}$. Au moment du test, ces
mêmes statistiques sont appliquées aux lignes de $\mathcal I_{\mathrm{te}}$ sans
nouvelle estimation.
...
l'apprentissage. On obtient ainsi $720+80=800$ lignes d'apprentissage et
$180+20=200$ lignes de test. La stratification ne modifie pas le déséquilibre
$90\,\%/10\,\%$ ; elle le reproduit dans les deux sous-ensembles.
...
Un \emph{hyperparamètre} est fixé par le protocole et non estimé directement par
le gradient. Par exemple, les coefficients $\thv$ sont des paramètres appris ;
...
$0{,}80$, on retient la deuxième avant d'évaluer une seule fois le modèle sur le
test. La validation croisée, construite explicitement au
chapitre~\ref{ch:evaluation}, généralise ce principe lorsque l'on ne souhaite pas
réserver un unique ensemble de validation.
