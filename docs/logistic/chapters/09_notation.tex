...
\begin{lecture}
Une formule se lit d'abord comme une phrase : quels objets entrent dans le
calcul, quelle opération les relie et quel objet en sort ? Les tableaux de ce
chapitre donnent pour chaque symbole sa nature, sa lecture orale professionnelle
et son rôle dans le modèle.
\end{lecture}

Ce chapitre fixe non seulement le sens des symboles, mais aussi la manière de les
nommer à l'oral. Une notation ne peut être comprise indépendamment de sa nature :
un scalaire, un vecteur, une matrice, une fonction et un ensemble ne se manipulent
pas de la même façon. Les vecteurs sont écrits en gras, les ensembles en lettres
calligraphiques et les paramètres grecs en gras lorsqu'ils forment un vecteur.

Le document adopte la convention statistique suivante : l'indice d'observation
est placé en bas. Ainsi, $\x_i$ est le vecteur de l'élève $i$, $x_{ij}$ sa
caractéristique $j$ et $c_i$ sa classe. Certains cours de machine learning
écrivent $\x^{(i)}$ afin de distinguer le numéro d'exemple d'une puissance ; cette
convention est légitime, mais elle est moins naturelle à l'oral et entre en conflit
avec l'usage matriciel $x_{ij}$. Elle n'est donc pas retenue ici. Les crochets en
exposant, comme $\thv^{[t]}$, sont réservés au numéro d'itération et ne désignent
...
\end{tikzpicture}}
\caption{Nature des objets successifs. Ce schéma empêche notamment de confondre
une caractéristique, un score, une sortie logistique et une classe.}
...
majuscule grasse $\X$ désigne l'ensemble des observations sous forme matricielle.}
\end{table}
...
\bottomrule
\end{tabularx}
\caption{Une classe observée $c_i$ ne doit pas être confondue avec la cible
binaire $y_{ik}$ fabriquée pour le sous-problème OvR numéro $k$.}
\end{table}
...
z_{\thv}(\x) & « le score z, paramétré par thêta, évalué en x » ; souvent « le score en x » & score linéaire réel $\thv^\top\xt$. Il n'est ni une classe ni une probabilité. \\
...
q_k(\x) & « q indice k évalué en x » & sortie du classifieur binaire $k$, égale à $\sigma(\thv_k^\top\xt)$. Les $q_k$ OvR ne sont pas contraints de sommer à $1$. \\
...
\bottomrule
\end{tabularx}
\caption{Le score $z$, la probabilité binaire $p$ et la décision $\widehat y$ sont
trois objets distincts produits successivement par le modèle.}
\end{table}
...
\nabla J(\thv) & « le gradient de J en thêta » & vecteur des $n+1$ dérivées partielles de $J$ ; il pointe dans la direction locale de plus forte augmentation. « Nabla J » sert surtout à dicter la formule. \\
\nabla^2J(\thv) & « la Hessienne de J en thêta » & matrice des dérivées secondes ; elle décrit la courbure locale de la fonction objectif. « Nabla carré J » décrit la notation, moins bien l'objet. \\
...
alpha fois le gradient évalué en cette valeur ». Les crochets $[t]$ repèrent une
itération ; ils ne désignent pas une puissance.
...
\Argmax_k a_k & « l'argument du maximum sur k de a indice k » & valeur de l'indice $k$ pour laquelle $a_k$ est maximal ; le résultat est un indice, pas la valeur maximale elle-même. Dans l'usage, « arg max » est également courant. \\
