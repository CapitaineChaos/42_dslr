...
\centering
\footnotesize
\begin{tabular}{>{\raggedright\arraybackslash}p{32mm}>{\raggedright\arraybackslash}p{56mm}>{\raggedright\arraybackslash}p{36mm}}
\toprule
domaine & attendu à l'entrée & rappel disponible \\
\midrule
statistique descriptive & moyenne, médiane, écart-type, valeur manquante
  & \annexeref{ann:prerequis} \\
analyse & exponentielle, logarithme, fonction croissante
  & \annexeref{ann:prerequis} \\
géométrie & repère, droite, produit scalaire, norme
  & \annexeref{ann:prerequis}, \annexeref{ann:geometrie} \\
calcul différentiel & dérivée d'une fonction d'une variable
  & \annexeref{ann:prerequis} \\
programmation & Python : fonctions, listes, boucles, imports
  & aucun \\
\bottomrule
\end{tabular}
...
\begin{enumerate}
  \item calculer le score et la probabilité d'une observation à partir des
        coefficients et des statistiques de préparation ;
  \item écrire la perte d'entropie croisée sous une forme stable en double
        précision, et la contrôler en $z=\pm1000$ ;
  \item prédire le signe et l'ordre de grandeur d'une composante du gradient ;
  \item distinguer un protocole d'évaluation valide d'un protocole entaché de
        fuite du jeu de test ;
  \item lire une matrice de confusion et l'assortir d'une incertitude ;
  \item énoncer la portée exacte de la convexité ;
  \item dire à quelles conditions les sorties d'un classifieur un-contre-tous
        se lisent comme des probabilités de classe.
\end{enumerate}
