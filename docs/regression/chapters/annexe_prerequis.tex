...
\begin{itemize}
  \item $u\cdot v=0$ équivaut à $u\perp v$, les deux vecteurs étant non nuls ;
  \item $\|v\|\cos\theta$ est la longueur de la projection de $v$ sur la
        direction de $u$, comptée avec un signe ;
  \item multiplier $u$ par $\lambda>0$ multiplie $u\cdot v$ par $\lambda$ sans
        changer le signe ni la direction.
\end{itemize}
...
\begin{itemize}
  \item $-\nabla J$ est la direction de plus forte décroissance locale de $J$
        au point considéré ;
  \item $\nabla J$ est orthogonal à la ligne de niveau de $J$ passant par ce
        point.
\end{itemize}
