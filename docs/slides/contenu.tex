...
\newlength{\chainw}\setlength{\chainw}{3.94cm}
\newlength{\chainh}\setlength{\chainh}{3.6cm}
\pgfplotsset{
  chainaxis/.style={
    scale only axis,width=3.2cm,height=1.95cm,
...
% Le tracé d'une vignette occupe une boîte de gabarit fixe, obtenue en dilatant
% le rectangle d'axes d'une marge identique sur les quatre côtés : son centre
% est donc exactement celui du rectangle d'axes. Étiquettes et légendes
% débordent de cette boîte au lieu de la dilater, si bien qu'une carte étiquetée
% et une carte nue placent leur tracé au même endroit, sans dérive latérale ni
% verticale. L'argument optionnel reprend la hauteur d'axe lorsque la vignette
% s'écarte de la hauteur commune.
\newcommand{\chainplot}[2][1.95]{%
  \begin{tikzpicture}
    \useasboundingbox (-.30,-.30) rectangle (3.5,{#1+.30});
    #2
  \end{tikzpicture}}

% Clé de légende : la maison se lit à la fois par la teinte et par la forme du
% marqueur employé dans les nuages, carré pour l'une, disque pour l'autre.
\newcommand{\chainkey}[3]{%
  \tikz[baseline=-.5ex]{\node[#2,fill=#1,inner sep=0pt,minimum size=2.5pt]{};}%
  \kern1.3pt\textcolor{#1}{#3}}
\newcommand{\chainhouses}{%
  \chainkey{coursescore!88!ink}{rectangle}{\MicroGryffondorLabel}\kern5pt
  \chainkey{warning!88!ink}{circle}{\MicroSerpentardLabel}}

% Les deux ordonnées sont fixes pour les huit cartes : le bandeau de légende ne
% déplace pas le tracé, il occupe une bande qui lui est réservée en permanence.
\newcommand{\chaincard}[5]{%
  \hyperref[#1]{%
    \begin{tikzpicture}
      ...
      \node[inner sep=0pt] at (.5\chainw,1.775cm) {#4};
      \node[inner sep=0pt,font=\fontsize{5.4}{6}\selectfont]
        at (.5\chainw,.23cm) {#5};
    \end{tikzpicture}}}
...
\begin{tikzpicture}[
    ...
    \chainplot{%
      \begin{axis}[chainaxis,xmin=0,xmax=20,ymin=0,ymax=100,
        ...
        \addplot[only marks,mark=*,mark size=.9pt,warning!88!ink]
          coordinates {\MicroAllSerpentardCoords};
      \end{axis}}}{\chainhouses}};

  \node[slot] (score) at (4.66,4.8) {\chaincard{course-stage-start-2}{coursescore}{SCORE LINÉAIRE}{%
    \chainplot{%
      \begin{axis}[chainaxis,xmin=0,xmax=20,ymin=0,ymax=100]
        \addplot[only marks,mark=square*,mark size=.9pt,coursescore!88!ink]
  ...
  \node[slot] (sigmoide) at (9.32,4.8) {\chaincard{course-stage-start-3}{courseprobability}{SIGMOÏDE}{%
    \chainplot{%
  ...
  \node[slot] (decision) at (13.98,4.8) {\chaincard{course-stage-start-7}{coursedecision}{DÉCISION}{%
    \chainplot[1.1]{%
      \begin{axis}[chainaxis,height=1.1cm,xmin=0,xmax=1,ymin=0,ymax=1]
        ...
        \node[font=\tiny,text=coursedecision!88!ink,anchor=south,
  ...
  \node[slot] (gradient) at (4.66,0) {\chaincard{course-stage-start-5}{coursegradient}{GRADIENT}{%
    \chainplot{%
      \begin{axis}[chainaxis,xmin=\MicroSliceXmin,xmax=\MicroSliceXmax,
        ymin=\MicroSliceYmin,ymax=\MicroSliceYmax,
        xlabel={$w_{\mathrm{pot}}$},ylabel={$w_{\mathrm{vol}}$},
  ...
  \node[slot] (perte) at (9.32,0) {\chaincard{course-stage-start-4}{courseloss}{PERTE LOGISTIQUE}{%
    \chainplot{%
  ...
  \node[slot] (evaluation) at (13.98,0)
    {\chaincard{course-stage-start-8}{courseevaluation}{ÉVALUATION}{%
    \begin{tikzpicture}[x=1.28cm,y=.66cm,
      cell/.style={draw=white,line width=.7pt,minimum width=1.22cm,
        minimum height=.62cm,font=\tiny\bfseries,inner sep=0pt},
      tag/.style={font=\tiny,text=ink!62}]
      \useasboundingbox (-1.26cm,-.945cm) rectangle (2.54cm,1.605cm);
    ...
    \end{tikzpicture}}{%
      \chainkey{warning!85!ink}{rectangle}{N = négatif}\kern6pt
      \chainkey{rulecolor!75!ink}{rectangle}{P = positif}}};
...
\begin{frame}[t]{Séparation entre estimation et évaluation}
  ...
  \begin{tikzpicture}[node distance=9mm and 11mm,
    box/.style={draw=ink!28,rounded corners=2pt,minimum height=14mm,
      minimum width=30mm,align=center,font=\small,inner xsep=3mm},
    arr/.style={-{Latex[length=2mm]},thick,ink!58},
    lab/.style={font=\scriptsize,text=ink!62,align=left,inner sep=2.5pt}]
    \node[box,fill=coursedata!13] (all) {\MicroNTotal{} élèves};
    \node[box,fill=coursedata!22,right=of all,yshift=14mm] (train)
      {\MicroNTrain{} élèves\\estimation};
    \node[box,fill=courseevaluation!18,right=of all,yshift=-14mm] (test)
      {\MicroNTest{} élèves\\évaluation};
    \node[box,fill=courseupdate!15,right=of train] (fit)
      {médianes, moyennes,\\écarts types, coefficients};
    ...
    \draw[arr] (train) -- (fit);
    \draw[arr] (test) -- (eval);
    % Seule flèche qui ne transporte pas des observations mais les nombres
    % estimés : elle doit le dire, sans quoi elle se lit comme un lien de plus.
    \draw[arr] (fit) -- node[lab,right] {paramètres appliqués\\sans réestimation}
      (eval);
  \end{tikzpicture}
  \vfill
...
\begin{frame}[t]{Groupe d'estimation : \MicroNTrain{} élèves}
  ...
  \end{tabularx}
  \par\vspace{2mm}
  \begin{vigilance}[Donnée manquante à l'intérieur du groupe d'estimation]
    La note de \PotionsName{} de Ron Weasley est manquante. L'imputation
    intervient donc dans le groupe qui fixe les paramètres : la valeur substituée
    pèse sur les médianes, sur les moyennes et, par voie de conséquence, sur les
...
\begin{frame}[t]{Groupe d'évaluation : huit élèves}
  ...
  \end{tabularx}
  \vfill
  \begin{vigilance}[Donnée manquante dans le groupe d'évaluation]
    La note de \VolName{} de George Weasley est manquante. La standardisation
    exige une valeur numérique : l'imputation la précède nécessairement, et la
    valeur substituée provient du groupe d'estimation, jamais des observations
    à évaluer.
  \end{vigilance}
\end{frame}
...
\begin{frame}[t]{Compléter, puis standardiser une note manquante}
  ...
  \begin{columns}[T,onlytextwidth]
    \column{.47\textwidth}
      \begin{coursecallout}{coursedata!88!ink}{Valeurs de remplacement}
        \[
          m_{\mathrm{pot}}=\MicroMedianPotions,
          \qquad m_{\mathrm{vol}}=\MicroMedianVol.
        \]
        Ces médianes sont lues sur les seules notes \emph{observées} du groupe
        d'estimation ; le groupe d'évaluation n'y contribue pas.
      \end{coursecallout}
    \column{.47\textwidth}
      \begin{coursecallout}{courseevaluation!88!ink}{Substitutions et notes
        standardisées}
        \[
          \begin{aligned}
            P_{\text{Ron}}&\leftarrow m_{\mathrm{pot}}
              &\Longrightarrow x_{\mathrm{pot}}&=\MicroImputedStdPotions,\\
            V_{\text{George}}&\leftarrow m_{\mathrm{vol}}
              &\Longrightarrow x_{\mathrm{vol}}&=\MicroImputedStdVol.
          \end{aligned}
        \]
        En \PotionsName{}, médiane et moyenne coïncident : la substitution tombe
        au centre de l'échelle standardisée. En \VolName{}, la médiane excède la
        moyenne (\(\MicroMedianVol\) contre \(\MicroMeanVol\)), d'un tiers
        d'écart type.
      \end{coursecallout}
  \end{columns}
  \vfill
  ...
  % Médiane et centrage-réduction sont des opérations, non des états : elles
  % portent les flèches. Les posait-on en boîtes que le schéma se lirait comme
  % une note manquante engendrant une médiane.
  \begin{tikzpicture}[node distance=30mm,
    ...
    lab/.style={font=\scriptsize,text=ink!62,inner sep=3pt}]
    \node[box,fill=warning!12] (raw) {note manquante};
    \node[box,fill=rulecolor!16,right=of raw] (ready) {note complétée};
    \node[box,fill=courseprobability!14,right=of ready] (standard)
      {note standardisée};
    \draw[arr] (raw) -- node[lab,above] {médiane de la matière} (ready);
    \draw[arr] (ready) -- node[lab,above] {centrage et réduction} (standard);
  \end{tikzpicture}
  \end{adjustbox}
\end{frame}
...
\begin{frame}[t]{Les paramètres sont figés avant l'évaluation}
  ...
  \begin{tikzpicture}[node distance=5mm,
    ...
    \node[box,fill=coursedata!15,right=of row] (impute)
      {$m_{\mathrm{pot}}=\MicroMedianPotions$ ; $m_{\mathrm{vol}}=\MicroMedianVol$\\si une note manque};
    \node[box,fill=courseprobability!15,right=of impute] (scale)
      {$\mu_{\mathrm{pot}},\mu_{\mathrm{vol}}=\MicroMeanPotions,\MicroMeanVol$\\$s_{\mathrm{pot}},s_{\mathrm{vol}}=\MicroSdPotions,\MicroSdVol$};
